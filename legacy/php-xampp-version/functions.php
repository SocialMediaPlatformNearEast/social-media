...
function validate_birthday(?string $birthday, bool $required = false): array
{
    $birthday = trim((string) $birthday);
    if ($birthday === '') {
        if (!$required) {
            return ['valid' => true, 'age' => null, 'birthday' => null];
        }

        return ['valid' => false, 'message' => 'Please enter your birthday.'];
    }

    $date = DateTimeImmutable::createFromFormat('!Y-m-d', $birthday);
    $errors = DateTimeImmutable::getLastErrors();
    if (!$date || ($errors !== false && ($errors['warning_count'] > 0 || $errors['error_count'] > 0))) {
        return ['valid' => false, 'message' => 'Please enter a valid birthday.'];
    }
    ...
    if ($date > $today) {
        return ['valid' => false, 'message' => 'Birthday cannot be in the future.'];
    }

    $age = (int) $date->diff($today)->y;
    if ($age < 13) {
        return ['valid' => false, 'message' => 'You must be at least 13 years old to use this app.'];
    }

    if ($age > 120) {
        return ['valid' => false, 'message' => 'Please enter a realistic birthday.'];
    }

    return ['valid' => true, 'age' => $age, 'birthday' => $date->format('Y-m-d')];
}

function birthday_date_limits(): array
{
    $today = new DateTimeImmutable('today');
    return [
        'min' => $today->modify('-120 years')->format('Y-m-d'),
        'max' => $today->modify('-13 years')->format('Y-m-d'),
    ];
}

function is_default_profile_photo(?string $photoUrl): bool
{
    ...
    foreach (GENDER_PROFILE_DEFAULTS as $defaults) {
        if ($photoUrl === $defaults['profile_photo_url']) {
            return true;
        }
    }
...
function fetch_community_highlights(int $viewerId, int $limit = 8): array
{
    ...
    $stmt = db()->prepare(
        "SELECT
         ...
         LEFT JOIN profiles pr ON pr.user_id = u.id
         WHERE p.deleted_at IS NULL
         ORDER BY ((SELECT COUNT(*) FROM likes l WHERE l.post_id = p.id) * 3)
            + ((SELECT COUNT(*) FROM reposts r WHERE r.post_id = p.id) * 4)
...
function render_community_highlights_panel(array $viewer): void
{
    $highlights = fetch_community_highlights((int) $viewer['id']);
    ?>
    <aside class="right-rail community-highlights" aria-label="Community highlights">
      <section class="community-highlights-panel">
        <header>
          <span class="panel-kicker">Community</span>
          <h2>Highlights</h2>
        </header>
        <div class="community-highlights-list">
          <?php if (!$highlights): ?>
            <p class="community-highlight-empty">Popular posts will appear here once the community gets active.</p>
          <?php endif; ?>
          <?php foreach ($highlights as $item): ?>
            <a class="community-highlight-item" href="post.php?id=<?= (int) $item['id'] ?>">
              <div class="community-highlight-author">
                <?php render_avatar($item, 'small'); ?>
                <span>
                  <strong><?= h($item['display_name']) ?></strong>
                  <small>@<?= h($item['username']) ?> &middot; <?= h(time_ago($item['created_at'])) ?></small>
                </span>
              </div>
              <p><?= h(mb_strimwidth((string) $item['content'], 0, 120, '...')) ?></p>
              <div class="community-highlight-stats">
                <span><?= (int) $item['like_count'] ?> likes</span>
                <span><?= (int) $item['repost_count'] ?> reposts</span>
                <span><?= (int) $item['reply_count'] ?> replies</span>
              </div>
            </a>
          <?php endforeach; ?>
        </div>
