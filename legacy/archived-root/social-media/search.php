...
if ($query !== '') {
    if ($tab === 'people') {
        ...
        if ($tab === 'top') {
            usort($posts, static function (array $a, array $b): int {
                $scoreA = ((int) $a['like_count'] * 3) + ((int) $a['reply_count'] * 2) + (int) $a['repost_count'];
                $scoreB = ((int) $b['like_count'] * 3) + ((int) $b['reply_count'] * 2) + (int) $b['repost_count'];
                return $scoreB <=> $scoreA ?: strcmp((string) $b['created_at'], (string) $a['created_at']);
            });
        }
    }
} else {
    $suggestedStmt = db()->prepare(
        'SELECT u.*,
                COALESCE(pr.display_name, u.display_name, u.username) AS display_name,
                COALESCE(pr.profile_pic, u.theme_color, u.avatar_color, "#111111") AS avatar_color,
                COALESCE(pr.profile_photo_url, u.profile_photo_url, "") AS profile_photo_url,
    ...
    $suggestedStmt->execute([(int) $viewer['id'], (int) $viewer['id']]);
    $suggestedUsers = $suggestedStmt->fetchAll();
    $recentPosts = array_slice(fetch_posts((int) $viewer['id']), 0, 3);
}
...
<div class="search-page-container">
  <header class="page-header search-header">
    <div class="search-input-wrapper">
      <form action="search.php" method="get" class="search-page-form">
        ...
        <input type="hidden" name="f" value="<?= h($tab) ?>">
        <input type="search" name="q" value="<?= h($query) ?>" placeholder="Search LvL" autofocus>
      </form>
    </div>
  ...
  <?php if ($query === ''): ?>
    <div class="search-empty-state">
      ...
      <p>Find people, posts, and more on LvL.</p>
    </div>
        ...
        <?php foreach ($suggestedUsers as $user): ?>
          <div class="user-result-card">
            ...
            <div class="user-result-info">
              ...
              <p class="user-result-bio"><?= h($user['bio'] ?: 'New to LvL.') ?></p>
            </div>
          </div>
        <?php endforeach; ?>
      </section>
    <?php endif; ?>
    <?php if ($recentPosts): ?>
      <section class="search-discovery feed">
        <h3>Recent posts</h3>
