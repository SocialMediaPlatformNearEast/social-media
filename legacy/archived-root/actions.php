...
try {
    ...
    if ($action === 'register') {
        ...
        $birthdayValidation = validate_birthday($birthday);
        $age = $birthdayValidation['valid'] && $birthdayValidation['age'] !== null ? (int) $birthdayValidation['age'] : ($ageInput !== '' ? (int) $ageInput : null);
        ...
        if (!$birthdayValidation['valid']) {
            flash((string) $birthdayValidation['message'], 'error');
            redirect('auth.php');
        }
    ...
    if ($action === 'create_post') {
        ...
        award_xp((int) $viewer['id'], 'post_created', 10, (string) $postId);
    ...
    if ($action === 'update_profile') {
        ...
        $ageInput = trim((string) ($_POST['age'] ?? ''));
        $birthdayValidation = validate_birthday($birthday);
        $age = $birthdayValidation['valid'] && $birthdayValidation['age'] !== null
            ? (int) $birthdayValidation['age']
            : ($ageInput !== '' ? (int) $ageInput : (isset($viewer['age']) ? (int) $viewer['age'] : null));
        ...
        $currentPhotoUrl = (string) ($viewer['profile_photo_url'] ?? '');
        $profilePhotoUrl = is_default_profile_photo($currentPhotoUrl)
            ? (string) $profileDefaults['profile_photo_url']
            : $currentPhotoUrl;
        $stmt = db()->prepare(
            'INSERT INTO profiles (user_id, display_name, bio, profile_pic, profile_photo_url)
             VALUES (?, ?, ?, ?, ?)
             ON DUPLICATE KEY UPDATE display_name = VALUES(display_name), bio = VALUES(bio), profile_pic = VALUES(profile_pic), profile_photo_url = VALUES(profile_photo_url), updated_at = CURRENT_TIMESTAMP'
        );
        $stmt->execute([(int) $viewer['id'], $displayName, $bio, $color, $profilePhotoUrl ?: null]);

        $userStmt = db()->prepare('UPDATE users SET first_name = ?, last_name = ?, nickname = ?, username = ?, display_name = ?, bio = ?, avatar_color = ?, theme_color = ?, gender = ?, profile_photo_url = ?, age = ?, birthday = ?, location = ?, website = ? WHERE id = ?');
        $userStmt->execute([$firstName, $lastName, $nickname, $nickname, $displayName, $bio, $color, $color, $gender, $profilePhotoUrl ?: null, $age, $birthdayValidation['birthday'], $location, $website, (int) $viewer['id']]);
